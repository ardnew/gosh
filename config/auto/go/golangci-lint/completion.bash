...
__golangci-lint_handle_go_custom_completion()
{
    __golangci-lint_debug "${FUNCNAME[0]}: cur is ${cur}, words[*] is ${words[*]}, #words[@] is ${#words[@]}"

    local out requestComp lastParam lastChar comp directive args

    # Prepare the command to request completions for the program.
    # Calling ${words[0]} instead of directly golangci-lint allows to handle aliases
    args=("${words[@]:1}")
    requestComp="${words[0]} __completeNoDesc ${args[*]}"

    lastParam=${words[$((${#words[@]}-1))]}
    lastChar=${lastParam:$((${#lastParam}-1)):1}
    __golangci-lint_debug "${FUNCNAME[0]}: lastParam ${lastParam}, lastChar ${lastChar}"

    if [ -z "${cur}" ] && [ "${lastChar}" != "=" ]; then
        # If the last parameter is complete (there is a space following it)
        # We add an extra empty parameter so we can indicate this to the go method.
        __golangci-lint_debug "${FUNCNAME[0]}: Adding extra empty parameter"
        requestComp="${requestComp} \"\""
    fi

    __golangci-lint_debug "${FUNCNAME[0]}: calling ${requestComp}"
    ...
    __golangci-lint_debug "${FUNCNAME[0]}: the completion directive is: ${directive}"
    __golangci-lint_debug "${FUNCNAME[0]}: the completions are: ${out[*]}"

    if [ $((directive & 1)) -ne 0 ]; then
        # Error code.  No completion.
        __golangci-lint_debug "${FUNCNAME[0]}: received error from custom completion go code"
        return
    else
        if [ $((directive & 2)) -ne 0 ]; then
            if [[ $(type -t compopt) = "builtin" ]]; then
                __golangci-lint_debug "${FUNCNAME[0]}: activating no space"
                compopt -o nospace
            fi
        fi
        if [ $((directive & 4)) -ne 0 ]; then
            if [[ $(type -t compopt) = "builtin" ]]; then
                __golangci-lint_debug "${FUNCNAME[0]}: activating no file completion"
                compopt +o default
            fi
        fi
...
_golangci-lint_config_path()
{
    ...
    flags+=("--sort-results")
    local_nonpersistent_flags+=("--sort-results")
    flags+=("--path-prefix=")
    ...
    flags+=("--allow-parallel-runners")
    local_nonpersistent_flags+=("--allow-parallel-runners")
    flags+=("--allow-serial-runners")
    local_nonpersistent_flags+=("--allow-serial-runners")
    flags+=("--enable=")
    ...
    flags+=("--exclude-case-sensitive")
    local_nonpersistent_flags+=("--exclude-case-sensitive")
